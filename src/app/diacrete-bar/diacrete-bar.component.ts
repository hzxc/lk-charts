...
export class DiacreteBarComponent implements OnInit {
	...
	getRandWeightArray(){
		let weightLevel = [
			{ name: '1公斤以内', min: 0.0000001, max: 1 },
			{ name: '1---3公斤', min: 1, max: 3 },
			{ name: '3---5公斤', min: 3, max: 5 },
			{ name: '5---10公斤', min: 5, max: 10 },
			{ name: '大于10公斤', min: 10, max: 1000 },
			{ name: '--无重量--', min: 0, max: 0 }
		]
		this.weightArray[5] = ({ name: '--无重量--', value: 0 })
		for(let i=0;i<6;i++){
			this.weightArray[i] = ({ name: weightLevel[i]['name'], value: this._OrderServesService.getRand()*5});
		}
	...
	getWeightArray() {
		...
		for (let i = 0; i < weightLevel.length; i++) {
			this._OrderServesService
				.GetOrderAmountByWeight(this._OrderServesService.rCreateTime(), 'lk01', weightLevel[i].min, weightLevel[i].max)
				.subscribe((r1) => {
					this._OrderServesService
						.GetOrderAmountByWeight(this._OrderServesService.rCreateTime(), 'lk02', weightLevel[i].min, weightLevel[i].max)
						.subscribe((r2) => {
							this.weightArray[i] = ({ name: weightLevel[i]['name'], value: ((r1['result']['amount'] + r2['result']['amount']) * this.multiple) });
							let myChart = echarts.init(document.getElementById('radar'));
							this.chartsInit();
							myChart.setOption(this.option);
	...
	chartsInit() {
		...
		var array1 = [
			this.weightArray[0]['name'],
			this.weightArray[1]['name'],
			this.weightArray[2]['name'],
			this.weightArray[3]['name'],
			this.weightArray[4]['name'],
			this.weightArray[5]['name'],
		...
		this.option = {
			...
			yAxis: {
				...
				axisLabel: {
					...
					formatter: function (value) {
						// console.log('{' + value + '| }{value|' + array[value] + '}');  //{5| }{value|1公斤以内}
			...
			series: [{
				...
				data: [
					this.weightArray[0]['value'],
					this.weightArray[1]['value'],
					this.weightArray[2]['value'],
					this.weightArray[3]['value'],
					this.weightArray[4]['value'],
					this.weightArray[5]['value']
				]
			}]
		};
